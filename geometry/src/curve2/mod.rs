...
pub trait Curve2Impl {
    ...
    fn der2_normalized(&self, u: f64) -> Vec2 {
        self.der2(self.denormalize_u(u))
    }

    fn tangent_normalized(&self, u: f64) -> Vec2 {
        self.tangent(self.denormalize_u(u))
    }

    fn normal_normalized(&self, u: f64) -> Vec2 {
        self.normal(self.denormalize_u(u))
    }

    fn curvature_normalized(&self, u: f64) -> f64 {
        self.curvature(self.denormalize_u(u))
    }
}
...
mod tests {
    ...
    pub fn validate_der1<C: Curve2Impl>(curve: &C, samples: usize, tolerance: f64) {
        for i in 0..samples {
            ...
            for _ in 0..64 {
                ...
                let lo_pos = curve.eval_normalized(u_lo);
                let hi_pos = curve.eval_normalized(u_hi);
                ...
                let approx_der1 = (hi_pos - lo_pos) / (curve.denormalize_u(u_hi - u_lo));
    ...
    pub fn validate_der2<C: Curve2Impl>(curve: &C, samples: usize, tolerance: f64) {
        for i in 0..samples {
            ...
            let actual_der2 = curve.der2_normalized(u);
            ...
            for _ in 0..64 {
                ...
                let lo_der1 = curve.der1_normalized(u_lo);
                let hi_der1 = curve.der1_normalized(u_hi);

                // Approximate the second derivative by getting a vector between those two vectors
                // and scaling it by the parameter distance between them
                let approx_der1 = (hi_der1 - lo_der1) / (curve.denormalize_u(u_hi - u_lo));
